...
export default function AdminChatbotQuestions() {
  ...
  const [productSelectCategoryId, setProductSelectCategoryId] = useState("");
  const [productSelectSubcategoryId, setProductSelectSubcategoryId] = useState("");
  ...
  const [newQuestionProducts, setNewQuestionProducts] = useState([]);
  ...
  const fetchQuestions = async () => {
    try {
      const res = await apiClient.get("/api/chat/admin/product-questions");
  ...
  useEffect(() => {
    setIsLoading(true);
    Promise.all([fetchQuestions(), fetchCategories()]).finally(() => setIsLoading(false));
  }, []);

  useEffect(() => {
    if (productSelectCategoryId) fetchSubcategories(productSelectCategoryId);
    else {
      setSubcategories([]);
      setProducts([]);
    }
  }, [productSelectCategoryId]);

  useEffect(() => {
    if (productSelectSubcategoryId) fetchProducts(productSelectSubcategoryId);
    else setProducts([]);
  }, [productSelectSubcategoryId]);

  const addProductToNew = (product) => {
    if (!product?._id || newQuestionProducts.some((p) => p._id === product._id)) return;
    setNewQuestionProducts((prev) => [...prev, { _id: product._id, title: product.title }]);
  };

  const removeProductFromNew = (productId) => {
    setNewQuestionProducts((prev) => prev.filter((p) => p._id !== productId));
  };

  const addProductToEdit = (product) => {
    if (!product?._id || editProducts.some((p) => p._id === product._id)) return;
    setEditProducts((prev) => [...prev, { _id: product._id, title: product.title }]);
  };

  const removeProductFromEdit = (productId) => {
    setEditProducts((prev) => prev.filter((p) => p._id !== productId));
  };
  ...
  const createQuestion = async () => {
    ...
    try {
      const res = await apiClient.post("/api/chat/admin/product-questions", {
        questionText: text,
        sortOrder: questions.length,
        required: newQuestionRequired,
        answerType: newQuestionAnswerType,
        options: opts,
        products: newQuestionProducts.map((p) => p._id),
      });
  ...
  const updateQuestion = async () => {
    ...
    try {
      const res = await apiClient.put(`/api/chat/admin/product-questions/${editingId}`, {
        questionText: editText.trim(),
        required: editRequired,
        answerType: editAnswerType,
        options: opts,
        products: editProducts.map((p) => p._id),
      });
      setQuestions((prev) =>
  ...
  const deleteQuestion = async (id) => {
    try {
      await apiClient.delete(`/api/chat/admin/product-questions/${id}`);
  ...
  const startEdit = (q) => {
    ...
    setEditProducts(
      (q.products || []).map((p) =>
        typeof p === "object" && p?.title ? { _id: p._id, title: p.title } : { _id: p, title: String(p) }
      )
    );
  };
  ...
  const getProductTitle = (p) => (typeof p === "object" && p?.title ? p.title : String(p));

  return (
    <AdminShell
      testId="page-admin-chatbot-questions"
      title="Manage Questions"
      subtitle="Create and edit questions. Assign products to each question—users see these before entering contact details."
      sectionBare
    >
      {loadError ? (
              ...
              <div className="rounded-xl border border-border/70 bg-card p-4 space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. What quantity do you need?"
                ...
                <div>
                  <label className="mb-2 block text-xs font-medium text-muted-foreground">Assign to products</label>
                  <div className="flex flex-wrap items-end gap-2">
                    <div className="min-w-[140px]">
                      <select
                        value={productSelectCategoryId}
                        onChange={(e) => setProductSelectCategoryId(e.target.value)}
                        className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm"
                      >
                    ...
                    <div className="min-w-[140px]">
                      <select
                        value={productSelectSubcategoryId}
                        onChange={(e) => setProductSelectSubcategoryId(e.target.value)}
                        disabled={!productSelectCategoryId}
                    ...
                    <div className="min-w-[180px]">
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          if (p) addProductToNew(p);
                          e.target.value = "";
                        }}
                        disabled={!productSelectSubcategoryId}
                        className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm disabled:opacity-50"
                      >
                    ...
                    <span className="text-xs text-muted-foreground">Select category → subcategory → product</span>
                  </div>
                  {newQuestionProducts.length > 0 ? (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {newQuestionProducts.map((p) => (
                        <span
                          key={p._id}
                          className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2 py-0.5 text-xs"
                        >
                          {p.title}
                          <button type="button" onClick={() => removeProductFromNew(p._id)} className="hover:text-red-600">
                            <X className="h-3 w-3" />
                          </button>
                        </span>
                      ))}
                        ...
                        <div>
                          ...
                          {editProducts.length > 0 ? (
                            <div className="mt-1 flex flex-wrap gap-1">
                              {editProducts.map((p) => (
                                <span key={p._id} className="inline-flex items-center gap-1 rounded-full bg-primary/10 px-2 py-0.5 text-xs">
                                  {p.title}
                                  <button type="button" onClick={() => removeProductFromEdit(p._id)}><X className="h-3 w-3" /></button>
                                </span>
                              ))}
                            </div>
                          ) : null}
                      ...
                      <div className="flex items-center justify-between gap-4">
                        <div>
                          ...
                          {(q.products?.length ?? 0) > 0 ? (
                            ...
                            <p className="mt-0.5 text-xs text-muted-foreground">No products assigned</p>
                          )}
                        </div>
