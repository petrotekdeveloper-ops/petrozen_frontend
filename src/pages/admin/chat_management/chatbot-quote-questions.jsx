...
export default function AdminChatbotQuoteQuestions() {
  ...
  const fetchQuestions = async () => {
    try {
      const res = await apiClient.get("/api/chat/admin/quote-questions");
  ...
  const createQuestion = async () => {
    ...
    try {
      const res = await apiClient.post("/api/chat/admin/quote-questions", {
        questionText: text,
  ...
  const updateQuestion = async () => {
    ...
    try {
      const res = await apiClient.put(`/api/chat/admin/quote-questions/${editingId}`, {
  ...
  const deleteQuestion = async (id) => {
    try {
      await apiClient.delete(`/api/chat/admin/quote-questions/${id}`);
  ...
  return (
    <AdminShell
      testId="page-admin-chatbot-quote-questions"
      title="Quote Questions"
      subtitle="Manage questions shown to users who select Request a Quote. Questions appear after contact details (name, company, email) and before the thank-you message. No categories—answers are free-form or from options."
      sectionBare
    >
      {loadError ? (
              ...
              <div className="rounded-xl border border-border/70 bg-card p-4 space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. What is your expected quantity?"
          ...
          <div>
            ...
            {questions.length === 0 ? (
              <div className="rounded-xl border border-dashed border-border/60 bg-muted/20 px-6 py-12 text-center text-sm text-muted-foreground">
                No questions yet. Add one above. Users will go straight to the thank-you message after contact details if there are no questions.
              </div>
            ) : (
